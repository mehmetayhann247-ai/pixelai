      ...
      <div className="tool-container">
        <div style={{ textAlign: "center", marginBottom: "48px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(234,179,8,0.15)", border: "1px solid rgba(234,179,8,0.3)", borderRadius: "999px", padding: "6px 16px", fontSize: "12px", color: "#fbbf24", marginBottom: "20px" }}>
            ...
            AI Destekli — Fotoğraf Restore
          </div>
          <h1 className="tool-title" style={{ fontWeight: "900", marginBottom: "12px", letterSpacing: "-1.5px", background: "linear-gradient(135deg, #fff 0%, #e2e8f0 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Eski Fotoğraf Restore</h1>
          <p style={{ color: "#6b7280", fontSize: "17px" }}>Hasarlı ve solmuş eski fotoğrafları yapay zeka ile yenile</p>
        </div>
        ...
        {!preview && (
          <div onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) handleFile(f); }} onDragOver={e => e.preventDefault()} className="tool-drop"
            ...
            <label style={{ background: "linear-gradient(135deg, #b45309, #fbbf24)", color: "white", padding: "13px 36px", borderRadius: "12px", cursor: "pointer", fontSize: "16px", fontWeight: "700", boxShadow: "0 0 30px rgba(180,83,9,0.3)" }}>
              Fotoğraf Seç
        ...
        {preview && (
          <div>
            <div className="tool-grid">
              ...
              {result && (
                <div className="tool-col">
                  <p style={{ color: "#fbbf24", marginBottom: "10px", textAlign: "center", fontSize: "12px", fontWeight: "700", letterSpacing: "1px", textTransform: "uppercase" }}>Restore ✨</p>
            ...
            <div className="tool-btns" style={{ display: "flex", gap: "12px", justifyContent: "center" }}>
              ...
              {!result && <button onClick={handleSubmit} disabled={loading} style={{ background: loading ? "rgba(255,255,255,0.08)" : "linear-gradient(135deg, #b45309, #fbbf24)", color: loading ? "#6b7280" : "white", border: "none", borderRadius: "12px", padding: "13px 40px", fontSize: "15px", cursor: loading ? "not-allowed" : "pointer", fontWeight: "700" }}>{loading ? "⏳ Restore ediliyor..." : "🕰️ Restore Et"}</button>}
              {result && <a href={result} download="pixelai-restore.png" style={{ background: "linear-gradient(135deg, #059669, #10b981)", color: "white", padding: "13px 36px", borderRadius: "12px", textDecoration: "none", fontSize: "15px", fontWeight: "700" }}>⬇️ İndir</a>}
            </div>
          </div>
        )}
      </div>
