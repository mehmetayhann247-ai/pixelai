    ...
    <main style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f0f1a 0%, #1a0a2e 50%, #0d1117 100%)", color: "white", fontFamily: "sans-serif", padding: "0 0 60px" }}>
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "40px 20px" }}>
        <div style={{ textAlign: "center", marginBottom: "40px" }}>
          <div style={{ display: "inline-block", background: "rgba(234,179,8,0.2)", border: "1px solid #eab308", borderRadius: "999px", padding: "6px 18px", fontSize: "13px", color: "#fbbf24", marginBottom: "16px" }}>🕰️ AI Destekli</div>
          <h1 style={{ fontSize: "40px", fontWeight: "900", marginBottom: "12px" }}>Eski Fotoğraf Restore</h1>
          <p style={{ color: "#9ca3af", fontSize: "17px" }}>Eski ve hasarlı fotoğraflarını AI ile restore et</p>
        </div>
        ...
        {!preview && (
          <div onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) handleFile(f); }} onDragOver={e => e.preventDefault()}
            ...
            <p style={{ fontSize: "20px", fontWeight: "600", marginBottom: "24px" }}>Eski fotoğrafı sürükle bırak veya seç</p>
        ...
        {preview && (
          <div style={{ textAlign: "center" }}>
            <div style={{ display: "flex", gap: "20px", justifyContent: "center", flexWrap: "wrap", marginBottom: "24px" }}>
              ...
              {result && (
                <div style={{ flex: 1, minWidth: "250px" }}>
                  <p style={{ color: "#fbbf24", marginBottom: "8px", fontSize: "13px" }}>RESTORE EDİLMİŞ</p>
            ...
            <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
              ...
              {!result && <button onClick={handleSubmit} disabled={loading} style={{ background: loading ? "#555" : "linear-gradient(135deg, #eab308, #fbbf24)", color: "black", border: "none", borderRadius: "10px", padding: "12px 40px", fontSize: "16px", cursor: loading ? "not-allowed" : "pointer" }}>{loading ? "⏳ Restore ediliyor..." : "🕰️ Restore Et"}</button>}
              {result && <a href={result} download="pixelai-restored.png" style={{ background: "linear-gradient(135deg, #059669, #10b981)", color: "white", padding: "12px 32px", borderRadius: "10px", textDecoration: "none", fontSize: "16px" }}>⬇️ İndir</a>}
            </div>
          </div>
        )}
      </div>
