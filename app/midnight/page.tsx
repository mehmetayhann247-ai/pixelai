...
export default function Midnight() {
  ...
  const handleSubmit = async () => {
    ...
    formData.append("style", "midnight");
    ...
    <main style={{ minHeight: "100vh", background: "#080b14", color: "white", fontFamily: "'Segoe UI', sans-serif", paddingBottom: "60px" }}>
      ...
      <div className="tool-container">
        <div style={{ textAlign: "center", marginBottom: "48px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(79,70,229,0.15)", border: "1px solid rgba(79,70,229,0.3)", borderRadius: "999px", padding: "6px 16px", fontSize: "12px", color: "#818cf8", marginBottom: "20px" }}>
            <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#818cf8", display: "inline-block" }}></span>
            AI Destekli — Gece Sanatı
          </div>
          <h1 className="tool-title" style={{ fontWeight: "900", marginBottom: "12px", letterSpacing: "-1.5px", background: "linear-gradient(135deg, #fff 0%, #e2e8f0 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Gece Yarısı Sanat</h1>
          <p style={{ color: "#6b7280", fontSize: "17px" }}>Fotoğrafını karanlık ve mistik sanat stiline dönüştür</p>
        </div>
        ...
        {!preview && (
          <div onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) handleFile(f); }} onDragOver={e => e.preventDefault()} className="tool-drop"
            ...
            <label style={{ background: "linear-gradient(135deg, #312e81, #818cf8)", color: "white", padding: "13px 36px", borderRadius: "12px", cursor: "pointer", fontSize: "16px", fontWeight: "700", boxShadow: "0 0 30px rgba(49,46,129,0.4)" }}>
              Fotoğraf Seç
        ...
        {preview && (
          <div>
            <div className="tool-grid">
              ...
              {result && (
                <div className="tool-col">
                  <p style={{ color: "#818cf8", marginBottom: "10px", textAlign: "center", fontSize: "12px", fontWeight: "700", letterSpacing: "1px", textTransform: "uppercase" }}>Gece Yarısı ✨</p>
                  <div style={{ background: "rgba(79,70,229,0.08)", border: "1px solid rgba(79,70,229,0.3)", borderRadius: "16px", padding: "8px" }}>
            ...
            <div className="tool-btns" style={{ display: "flex", gap: "12px", justifyContent: "center" }}>
              <button onClick={() => { setPreview(""); setImage(null); setResult(""); }} style={{ background: "rgba(255,255,255,0.05)", color: "#818cf8", border: "1px solid rgba(79,70,229,0.3)", borderRadius: "12px", padding: "13px 28px", fontSize: "15px", cursor: "pointer", fontWeight: "600" }}>🔄 Yeni Fotoğraf</button>
              {!result && <button onClick={handleSubmit} disabled={loading} style={{ background: loading ? "rgba(255,255,255,0.08)" : "linear-gradient(135deg, #312e81, #818cf8)", color: loading ? "#6b7280" : "white", border: "none", borderRadius: "12px", padding: "13px 40px", fontSize: "15px", cursor: loading ? "not-allowed" : "pointer", fontWeight: "700" }}>{loading ? "⏳ Dönüştürülüyor..." : "🌃 Gece Yarısı Sanatına Dönüştür"}</button>}
              {result && <a href={result} download="pixelai-midnight.png" style={{ background: "linear-gradient(135deg, #059669, #10b981)", color: "white", padding: "13px 36px", borderRadius: "12px", textDecoration: "none", fontSize: "15px", fontWeight: "700" }}>⬇️ İndir</a>}
            </div>
          </div>
        )}
      </div>
