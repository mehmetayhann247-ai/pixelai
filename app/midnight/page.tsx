    ...
    <main style={{ minHeight: "100vh", background: "#080b14", color: "white", fontFamily: "sans-serif", padding: "0 0 60px" }}>
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "40px 20px" }}>
        <div style={{ textAlign: "center", marginBottom: "40px" }}>
          <div style={{ display: "inline-block", background: "rgba(79,70,229,0.15)", border: "1px solid rgba(79,70,229,0.3)", borderRadius: "999px", padding: "6px 18px", fontSize: "13px", color: "#818cf8", marginBottom: "16px" }}>🌃 AI Destekli</div>
          <h1 style={{ fontSize: "40px", fontWeight: "900", marginBottom: "12px" }}>Gece Yarısı Sanat</h1>
          <p style={{ color: "#9ca3af", fontSize: "17px" }}>Fotoğrafına karanlık sanatsal efekt uygula</p>
        </div>
        ...
        {!preview && (
          <div onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) handleFile(f); }} onDragOver={e => e.preventDefault()}
            ...
            <label style={{ background: "#4f46e5", color: "white", padding: "12px 32px", borderRadius: "10px", cursor: "pointer", fontSize: "16px" }}>
              Fotoğraf Seç
        ...
        {preview && (
          <div style={{ textAlign: "center" }}>
            <div style={{ display: "flex", gap: "20px", justifyContent: "center", flexWrap: "wrap", marginBottom: "24px" }}>
              ...
              {result && (
                <div style={{ flex: 1, minWidth: "250px" }}>
                  <p style={{ color: "#818cf8", marginBottom: "8px", fontSize: "13px" }}>GECE YARISI</p>
                  <img src={result} style={{ width: "100%", borderRadius: "12px", border: "1px solid #4f46e5" }} />
                </div>
              )}
            </div>
            <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
              <button onClick={() => { setPreview(""); setImage(null); setResult(""); }} style={{ background: "transparent", color: "#818cf8", border: "1px solid #4f46e5", borderRadius: "10px", padding: "12px 24px", fontSize: "16px", cursor: "pointer" }}>🔄 Yeni Fotoğraf</button>
              {!result && <button onClick={handleSubmit} disabled={loading} style={{ background: loading ? "#555" : "linear-gradient(135deg, #4f46e5, #7c3aed)", color: "white", border: "none", borderRadius: "10px", padding: "12px 40px", fontSize: "16px", cursor: loading ? "not-allowed" : "pointer" }}>{loading ? "⏳ Dönüştürülüyor..." : "🌃 Sanat Efekti Uygula"}</button>}
              {result && <a href={result} download="pixelai-midnight.png" style={{ background: "linear-gradient(135deg, #059669, #10b981)", color: "white", padding: "12px 32px", borderRadius: "10px", textDecoration: "none", fontSize: "16px" }}>⬇️ İndir</a>}
            </div>
          </div>
        )}
      </div>
